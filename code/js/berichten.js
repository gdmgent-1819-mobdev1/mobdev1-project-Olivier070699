...
function read_data(){
  ...
  raw.on("value", function(snapshot){
    ...
    snapshot.forEach(function (childSnapshot){
      ...
      if (data.from == name) {
      	let post_content = "<div><h3>" + data.onderwerp + "</h3><button id='" + childSnapshot.key + "' class='readMessageBtn'>Open chat</button></div>";
...
function fullConversation(event){
	...
	window.location.href = "berichtFullConversationHuurder.html"
}
